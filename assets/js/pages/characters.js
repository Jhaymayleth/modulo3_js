...
export async function renderCharacters(page = 1) {
    ...
    document.getElementById('btn-save').addEventListener('click', () => {
        ...
        if (editId) {
            ...
            const idx = localCharacters.findIndex(c => String(c.id) === editId);

            if (idx !== -1) {
             
                localCharacters[idx] = { ...localCharacters[idx], name, status, species, image };
            } else {
           
                const oldCard = container.querySelector(`[data-id="${editId}"]`);
    ...
    document.getElementById('previous').addEventListener('click', () => {
        if (page > 1) renderCharacters(page - 1);
    });

    document.getElementById('next').addEventListener('click', () => {
        renderCharacters(page + 1);
    });
}